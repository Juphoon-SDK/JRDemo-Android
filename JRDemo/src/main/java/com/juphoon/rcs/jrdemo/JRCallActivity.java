        ...
        mOperationLayer.setEventReceiver(new OperationLayer.EventReceiver() {
            ...
            public void onEvent(String event, String... args) {
                if (OperationLayer.EVENT_SHOWN.equals(event)) {
                    ...
                    if (mStatistics == null) {
//                        JRLog
                        mStatistics = new Statistics(getApplicationContext(), mCurItem.getCallId());
...
//            }
        ...
        if (updateType == JRCallConstants.UPDATE_TYPE_CALL_TALKING) {
            if (item.isVideo() && !item.isConference()) {
                if (mRemoteCanvas == null) {
                    mRemoteCanvas = JRMediaDevice.getInstance().startVideo(mCurItem.getMemberList().get(0).getRenderId(), JRMediaDeviceContancts.VIDEO_CAMERA_FRONT);
    ...
    public void onRenderStart(SurfaceView surfaceView) {
        ...
        if (mRemoteCanvas != null) {
            if (mRemoteCanvas.getVideoView() == surfaceView) {
                //缩小本地窗口方法
                shrinkPreviewSurfaceView(mLocalCanvas.getVideoView(), mCurItem.getCallId());
            }
        }
    }
...
//        }
        if (mAdapter == null) {
            mAdapter = new JRMultiMemberAdapter(this, item.getMemberList(), addNumber,mIsSender);
