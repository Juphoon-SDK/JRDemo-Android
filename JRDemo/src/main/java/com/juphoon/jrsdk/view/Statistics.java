package com.juphoon.jrsdk.view;
...
public class Statistics extends FrameLayout implements OnClickListener, Runnable {
    ...
    private static JRCallMember mMember;
    ...
    public Statistics(Context context, JRCallMember member) {
        super(context);
        mMember = member;
    ...
    private static String getVoiceStat() {
        String s = JRCall.getInstance().getStatistics(false, mMember);
    ...
    private static String getVideoStat() {
        String s = JRCall.getInstance().getStatistics(true, mMember);
        if (TextUtils.isEmpty(s))
            s = mStrNotRunning;
        return s;
    }
