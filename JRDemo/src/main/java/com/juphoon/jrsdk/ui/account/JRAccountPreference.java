...
import com.juphoon.jrsdk.AccountAdvancedActivity;
...
public class JRAccountPreference extends PreferenceFragment implements OnPreferenceChangeListener, OnPreferenceClickListener {
    ...
    private static final String ACCOUNT_ADVANCED_MESSAGE = "account_advanced_message";
    ...
    public boolean onPreferenceClick(Preference preference) {
        ...
        if (preference.getKey().equals(ACCOUNT_ADVANCED_SIP)) {
            ...
            startActivity(intent);
            return true;
        } else if (preference.getKey().equals(ACCOUNT_ADVANCED_MESSAGE)) {
            intent.putExtra(AccountAdvancedActivity.ACCOUNT_ADVANCED_KEY, JRAccountAdvancedPreference.ADVANCED_TYPE_MESSAGE);
            intent.putExtra(ADVANCED_ACCOUNT, mAccount);
            startActivity(intent);
            return true;
        }
    ...
    private void loadData() {
        ...
        String name = account.getAccountConfig(mAccount, JRAccountDefine.ConfigKey.UserName);
        setPreString(ACCOUNT_USERNAME, name);
        setPreString(ACCOUNT_PASSWORD, account.getAccountConfig(mAccount, JRAccountDefine.ConfigKey.Password));
        setPreString(ACCOUNT_AUTHNAME, account.getAccountConfig(mAccount, JRAccountDefine.ConfigKey.AuthName));
        setPreString(ACCOUNT_REGIP, account.getAccountConfig(mAccount, JRAccountDefine.ConfigKey.RegIp));
        setPreString(ACCOUNT_REGREALM, account.getAccountConfig(mAccount, JRAccountDefine.ConfigKey.RegRealm));
        int intTmp = Integer.parseInt(account.getAccountConfig(mAccount, JRAccountDefine.ConfigKey.SipTransportType));
        ...
        setPreString(ACCOUNT_SIP_PORT, account.getAccountConfig(mAccount, JRAccountDefine.ConfigKey.SipPort));
        setPreBoolean(ACCOUNT_SIP_INSTANCE_ENABLE, Boolean.parseBoolean(account.getAccountConfig(mAccount, JRAccountDefine.ConfigKey.SipInstanceEnable)));
    }
    ...
    private void setStringValue(Preference preference, String value) {
        ...
        if (key.equals(ACCOUNT_USERNAME)) {
            account.setAccountConfig(mAccount, JRAccountDefine.ConfigKey.UserName, value);
        } else if (key.equals(ACCOUNT_PASSWORD)) {
            account.setAccountConfig(mAccount, JRAccountDefine.ConfigKey.Password, value);
        } else if (key.equals(ACCOUNT_AUTHNAME)) {
            account.setAccountConfig(mAccount, JRAccountDefine.ConfigKey.AuthName, value);
        } else if (key.equals(ACCOUNT_REGIP)) {
            account.setAccountConfig(mAccount, JRAccountDefine.ConfigKey.RegIp, value);
        } else if (key.equals(ACCOUNT_REGREALM)) {
            account.setAccountConfig(mAccount, JRAccountDefine.ConfigKey.RegRealm, value);
        } else if (key.equals(ACCOUNT_SIP_PORT)) {
            account.setAccountConfig(mAccount, JRAccountDefine.ConfigKey.SipPort, value);
        } else if (key.equals(ACCOUNT_SIP_TRANSPORT_TYPE)) {
            ListPreference p = (ListPreference) findPreference(key);
            p.setValue(value);
            account.setAccountConfig(mAccount, JRAccountDefine.ConfigKey.RegType, String.valueOf(p.findIndexOfValue((String) value)));
        }
    }
    ...
    private void setBooleanValue(Preference preference, boolean value) {
        ...
        if (key.equals(ACCOUNT_SIP_INSTANCE_ENABLE)) {
            account.setAccountConfig(mAccount, JRAccountDefine.ConfigKey.SipInstanceEnable, String.valueOf(value));
        }
    }
