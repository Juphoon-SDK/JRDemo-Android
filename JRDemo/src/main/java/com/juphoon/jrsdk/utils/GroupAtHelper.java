...
public class GroupAtHelper {
    private static GroupAtHelper mInstance;

    public static GroupAtHelper getInstance(){
        if(mInstance == null){
            mInstance = new GroupAtHelper();
        }
        return mInstance;
    }

    public interface IGroupAtHelperListener {
    ...
    private IGroupAtHelperListener mIGroupAtHelperListener;
    private static Map<String, String> sMapAtPhones = new HashMap<>(); // 保存全局的At队列，key表示某条消息的唯一标示

    public void setListener(IGroupAtHelperListener listener) {
        mIGroupAtHelperListener = listener;
    }
    ...
    private void notifyChooseAt() {
        if (mIGroupAtHelperListener != null) {
            new Handler().post(new Runnable() {

                @Override
                public void run() {
                    mIGroupAtHelperListener.onChooseAt();
                }
            });
        }
    }
